# normal_map_demo.gd — Demonstrates tangent-space normal mapping.
#
# WHAT:  Side-by-side comparison of surfaces with and without normal maps.
#        A brick-pattern normal map is applied to both a plane and a sphere,
#        revealing lighting detail that would otherwise require dense geometry.
# WHY:   Validates Phase 1.6 (Normal Map Support) — per-vertex tangent extraction,
#        TBN matrix construction, and tangent-space perturbation in the CPU shading pass.
#
# SCENE LAYOUT:
#   Node3D (this script)
#   ├── Camera3D
...
#   ├── PlaneNormalMapped    (brick normal map applied)
#   ├── PlaneFlat            (same albedo, no normal map)
#   ├── SphereNormalMapped   (brick normal map applied)
#   └── SphereFlat           (same albedo, no normal map)
#
# CONTROLS:
...
func _ready() -> void:
	...
	# Sun direction, color, energy are configured on the DirectionalLight3D node
	# in the .tscn — RayRenderer reads them per frame (Godot-Native principle).
	# Sky, ambient, and tonemapping live on the WorldEnvironment node.
	...
	tooltip.hint_text = (
		"[Normal Map Demo]\n"
		+ "Left pair = flat shading | Right pair = brick normal map\n\n"
	...
	print("[NormalMapDemo] Registered %d meshes, %d triangles" % [
